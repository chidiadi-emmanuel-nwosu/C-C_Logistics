...
class BaseModel():
    ...
    def to_dict(self):
        ...
        if "created_at" in new:
            new['created_at'] = (
                    new['created_at'].strftime("%Y-%m-%d %H:%M:%S.%f"))
        if "updated_at" in new:
            new['updated_at'] = (
                    new['updated_at'].strftime("%Y-%m-%d %H:%M:%S.%f"))
        if "date_of_birth" in new:
            new['date_of_birth'] = (
                    new['date_of_birth'].strftime("%Y-%m-%d %H:%M:%S.%f"))
        if "password" in new:
            del new['password']
        if "_sa_instance_state" in new:
            del new['_sa_instance_state']
        if "license_image_file" in new:
            del new['license_image_file']
